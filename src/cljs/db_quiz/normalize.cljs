...
(def name->regex
  ...
  (let [name-regex (str uppercase lowercase "+")
        apostrophes-regex "[\"']?"]
    (fn [text]
...
(defn despoilerify
  ...
  (let [despoilerify? (get-in @app-state [:options :despoilerify?])
        ...
        labels [label clean-label]
        ; Filter empty and < 2 characters long surface forms
        filter-surface-form (every-pred (complement (partial re-matches #"^\s+$"))
                                        (comp (partial < 2) count))
        ; Expand name-like labels into regular expressions allowing middle name.
        expand-name (fn [& names] (map name->regex (filter name-like? names)))
        ; Sort surface forms from the longest to the shortest, so that we first replace
        ; the longer matches.
        surface-forms (if despoilerify?
                          (concat (expand-name label clean-label)
                                  (sort-by (comp - count)
                                           (distinct (concat labels
                                                             (filter filter-surface-form
                                                                     (split surfaceForms "|"))))))
                          labels)]
    (assoc item
